...
from app.core.openai_review import openai_review_final, openai_review_final_boolean, openai_review_relevant_data
...
async def get_review_desc(request_id: str):
    try:
        ...
        aproval = await openai_review_final_boolean(final_inference)
        relevant_data = await openai_review_relevant_data(review_desc)
        if not review_desc:
